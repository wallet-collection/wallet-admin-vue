  ...
  <el-container class="wrapper">
    ...
    <el-container class="container-box" v-bind:class="{'slide-hide': isCollapse, 'slide-in-left': menuShow}">
      <el-header class="header">
        ...
        <div class="header-right">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item><span @click="handleFormGoogleForm">修改谷歌验证器</span></el-dropdown-item>
        ...
        <el-dialog
            title="修改谷歌验证码"
            :visible.sync="googleFormVisible"
            :before-close="hideGoogleForm"
            width="85%"
            top="5vh">
          <el-form :model="googleForm" :rules="googleFormRules" ref="dataGoogleForm">
            <el-form-item label="谷歌密钥" prop="secret">
              <div>
                <b style="color: red">科学上网后</b>
                下载Google验证码APP：
                <a target="_blank" href="https://apkcombo.com/apk-downloader/#package=com.google.android.apps.authenticator2">下载APP</a>
                ，
                或者安装浏览器插件：
                <a target="_blank" href="https://chrome.google.com/webstore/detail/authenticator/bhghoamapcdpbohphigoooaddinpkbai?hl=zh-CN">下载插件</a>
              </div>
              <div style="margin-top: 10px">
                打开APP后，在APP内扫描下方二维码绑定或者输入绑定码：{{googleForm.secret}}
              </div>
              <div style="text-align: center;margin-top: 10px">
                <img style="width: 200px;height: 200px;" :src="googleForm.qrCodeUrl" alt="">
              </div>
            </el-form-item>
            <el-form-item label="旧谷歌验证码，没有设置过则不填" prop="oldCode">
              <el-input v-model="googleForm.oldCode" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="新谷歌验证码" prop="code">
              <el-input v-model="googleForm.code" auto-complete="off"></el-input>
            </el-form-item>
          </el-form>
          <div slot="footer" class="dialog-footer">
            <el-button @click.native="hideGoogleForm">取消</el-button>
            <el-button type="primary" @click.native="googleFormSubmit()" :loading="googleFormLoading">提交</el-button>
          </div>
        </el-dialog>
...
import {GoogleAuthQrcode, SetGoogleAuthQrcode} from "../../api/auth/login";
...
export default {
  ...
  computed: {
    ...mapGetters(["sidebar", "adminInfo", "routers"]),
    onRoutes() {
      return this.$route.path;
    },
    username() {
      let username = this.adminInfo.username;
  ...
  methods: {
    ...
    hideGoogleForm() {
      // 更改值
      this.googleFormVisible = !this.googleFormVisible;
      return true;
    },
    handleFormGoogleForm() {
      this.googleFormVisible = true
      GoogleAuthQrcode()
          .then(res => {
            if (res.code) {
            ...
            this.googleForm.secret = res.data.secret
            this.googleForm.qrCode = res.data.qrCode
            this.googleForm.qrCodeUrl = res.data.qrCodeUrl
          })
          .catch(() => {
            this.googleFormLoading = false
            this.$message.error("网络错误，请稍后")
          })
    },
    googleFormSubmit() {
      this.$refs["dataGoogleForm"].validate(async valid => {
        if (valid) {

          this.googleFormLoading = true
          SetGoogleAuthQrcode(this.googleForm)
              .then(res => {
                this.googleFormLoading = false
                if (res.code) {
                  this.$message.error(res.message);
